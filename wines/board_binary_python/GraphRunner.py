...
def app(input_dir, output_dir, xmodel_file):
    # create graph runner
    ...
    output_list = []
    output_data = []
    print(f'Input Tensors: {input_tensor_buffers}')
    print(f'Output Tensors: {output_tensor_buffers}')
    for output_tensor_buffer in output_tensor_buffers:
        print(output_tensor_buffer)
        output_tensor_shape = output_tensor_buffer.get_tensor().dims
        output_data.append(np.zeros(output_tensor_shape, dtype='float32'))
    # run graph runner
    for i in range(len(input_data)):
        ...
        for inputTensor in input_tensor_buffers:
            tensor_shape = inputTensor.get_tensor().dims
            inputData.append(np.array(input_data[i]).reshape(tensor_shape))
